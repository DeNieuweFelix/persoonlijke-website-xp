...
function runZScode(){
    ...
    for(const x of allSyntax){
        ...
        const lineStr = x.join(" ");
        if(lineStr.trim().startsWith("<<") && lineStr.trim().endsWith(">>")){
        ...
        if(x[0].includes("SetVar")){
        ...
        }else if(x[0].includes("Meowify")){
            vars = "MEOW MEOW MEOW MEOW!!!";

            console.log(vars);
        }else if(x[0].includes("STRrep")){
            ...
            if(!STRtip){
                ZSoutput("TIP: use ֍ with STRrep to add spaces!");
                ZSoutput("Example: STRrep(str0) - ֍ ~");
            }

            STRtip = true;

            var a = (x[0].replace("STRrep(", "")).replace(")", "");
            ...
            if(c != 1){
                ZSerror("STRrep can only be used on strings!!!");
                return;
            }

            let curValue = vars[c][b];
            let newValue;

            console.log("current value: " + curValue);

            if(x[2] == "֍"){
                newValue = curValue.replaceAll(x[1], " ");
            }else{
                newValue = curValue.replaceAll(x[1], x[2]);
            }
